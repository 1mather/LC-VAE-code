...
class UCF101Images(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Sampling video frames (robust to short clips)
        start_frame_ind, end_frame_ind = self.temporal_sample(total_frames)
        span = end_frame_ind - start_frame_ind
        if span >= self.target_video_len:
            ...
            if total_frames >= self.target_video_len:
                ...
                base = np.linspace(0, max(total_frames - 1, 0), max(total_frames, 1), dtype=int)
                pad_len = self.target_video_len - len(base)
                pad_vals = np.full((pad_len,), base[-1] if len(base) > 0 else 0, dtype=int)
                frame_indice = np.concatenate([base, pad_vals], axis=0)
