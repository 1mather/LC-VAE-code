...
def distributed_generate_eval_subset(step,ema, vae, args, device, gen_dir, rank, world_size, per_channel_scale):
    ...
    for i in range(0, len(my_indices), per_batch):
        ...
        assert F_new == 8 and C_new == 16 and H_new == 32 and W_new == 32
        ...
        # 先把 32 拆回 16 * 2
        latents = latents.view(B, 8, 16, 16, 2, 16, 2)      # (B, 8, 16, 16, 2, 16, 2)
        # 反过来恢复到 (B, 8, 16, 2, 2, 16, 16)
        latents = latents.permute(0, 1, 2, 4, 6, 3, 5).contiguous()         # (B, 8, 16, 2, 2, 16, 16)

        # 合并 8 * 16 * 2 * 2 = 512，并恢复 F=1
        latents = latents.view(B, 1, 512, 16, 16)          # (B, 1, 512, 16, 16)
        ...
        _decode_and_save_batch(step,latents, vae, args, device, gen_dir, chunk)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                if getattr(args, 'vae_latent_scale', None) is not None and vae_latent_scale is None:
                ...
                assert F == 1 and C == 512 and H == 16 and W == 16

                # 512 = 8 * 16 * 2 * 2
                # 直接把 1*512 展开成 (8, 16, 2, 2)，最后两个 2 用来放大空间 16->32
                x = x_latent.view(B, 8, 16, 2, 2, 16, 16)           # (B, 8, 16, 2, 2, 16, 16)

                # 重新排列维度，把 2,2 插到 H,W 前面
                x = x.permute(0, 1, 2, 5, 3, 6, 4).contiguous()    # (B, 8, 16, 16, 2, 16, 2)
                x = x.view(B, 8, 16, 32, 32)                       # (B, 8, 16, 32, 32)
