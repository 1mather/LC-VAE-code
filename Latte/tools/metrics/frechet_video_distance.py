...
def compute_fvd(opts, max_real: int, num_gen: int, num_frames: int, realdata_subsample_factor: int=3, gendata_subsample_factor: int=1):
    ...
    # IMPORTANT: make the real-data clip selection deterministic.
    # If real videos are longer than `num_frames * subsample_factor`, the dataset loader can otherwise pick a random offset,
    # which makes FVD fluctuate across runs even with identical inputs.
    opts.dataset_kwargs.load_n_consecutive_random_offset = False
