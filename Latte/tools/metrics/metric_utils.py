...
def compute_feature_stats_for_dataset(
    ...
    # IMPORTANT: deterministic subset selection to make metrics reproducible.
    # Using `random.sample(...)` without a fixed seed makes IS/FVD fluctuate across runs even if inputs are unchanged.
    rng = np.random.RandomState(42)
    item_subset = rng.choice(data_length, size=num_items, replace=False).tolist()
