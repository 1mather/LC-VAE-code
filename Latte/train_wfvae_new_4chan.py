    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                assert F == 1 and C == 128 and H == 16 and W == 16

                # 512 = 8 * 16 * 2 * 2
                # 直接把 1*128 展开成 (8, 4, 2, 2)，最后两个 2 用来放大空间 16->32
                x = x_latent.view(B, 8, 4, 2, 2, 16, 16)           # (B, 8, 16, 2, 2, 16, 16)

                # 重新排列维度，把 2,2 插到 H,W 前面
                x = x.permute(0, 1, 2, 5, 3, 6, 4).contiguous()    # (B, 8, 4, 16, 2, 16, 2)
                x = x.view(B, 8, 4, 32, 32)                       # (B, 8, 16, 32, 32)
