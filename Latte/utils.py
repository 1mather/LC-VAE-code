...
def setup_distributed(backend="nccl", port=None):
    ...
    Supports both SLURM and torchrun (torch.distributed.run) environments,
    and falls back to single-process if none are detected.
    """
    num_gpus = torch.cuda.device_count()

    # Prefer torchrun-style env if present
    if "RANK" in os.environ and "WORLD_SIZE" in os.environ:
        rank = int(os.environ["RANK"])
        world_size = int(os.environ["WORLD_SIZE"])
        local_rank = int(os.environ.get("LOCAL_RANK", rank % max(1, num_gpus)))

        # Ensure master address/port are set for env:// init
        if "MASTER_ADDR" not in os.environ:
            os.environ["MASTER_ADDR"] = "127.0.0.1"
        if "MASTER_PORT" not in os.environ:
            os.environ["MASTER_PORT"] = str(port if port is not None else (29567 + num_gpus))

        os.environ["LOCAL_RANK"] = str(local_rank)

    # SLURM environment (when launched via srun/sbatch)
    elif "SLURM_PROCID" in os.environ and "SLURM_NTASKS" in os.environ:
        rank = int(os.environ["SLURM_PROCID"])
        world_size = int(os.environ["SLURM_NTASKS"])
        node_list = os.environ.get("SLURM_NODELIST", "")
        addr = subprocess.getoutput(f"scontrol show hostname {node_list} | head -n1") if node_list else "127.0.0.1"
        # Specify master port/address
        if port is not None:
        ...
        os.environ["LOCAL_RANK"] = str(rank % max(1, num_gpus))
