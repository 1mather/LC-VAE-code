...
class LPIPSWithDiscriminator3D(nn.Module):
    ...
    def forward(
        ...
        if optimizer_idx == 0: # Generator
            ...
            if aux_reconstructions is not None and aux_weight > 0.0:
                aux_reconstructions = rearrange(
                    aux_reconstructions, "b c t h w -> (b t) c h w"
                ).contiguous()
                aux_rec_loss = self.loss_func(inputs, aux_reconstructions)
                aux_nll_loss = aux_rec_loss / torch.exp(self.logvar) + self.logvar
                aux_nll_loss = torch.sum(aux_nll_loss) / aux_nll_loss.shape[0]
            else:
                aux_nll_loss = torch.tensor(0.0, device=inputs.device, dtype=inputs.dtype)
            ...
            if self.kl_weight > 0 and posteriors is not None:
            ...
            if teacher_latents is not None:
                ...
                # distill_loss = 1.0 - torch.nn.functional.cosine_similarity(
                #     student_latents.flatten(1), teacher_latents.flatten(1), eps=1e-6
                # ).mean()
            ...
            loss = (
                ...
                + consistence_weight * consistence
                + aux_weight * aux_nll_loss
            )
            log = {
                ...
                "{}/consistence_loss_weighted".format(split): consistence_weight * consistence.mean().detach().mean(),
                "{}/aux_nll_loss".format(split): aux_nll_loss.detach().mean(),
            }
            return loss, log
