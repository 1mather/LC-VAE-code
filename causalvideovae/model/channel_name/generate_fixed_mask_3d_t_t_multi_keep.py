...
def generate_fixed_mask_from_stats(mask_stats_file, handcraft, channel_num=32, erase_ratio=0.5, output_file=None):
    """
    从统计数据生成固定的通道mask
    
    Args:
        mask_stats_file: 统计数据JSON文件路径
        handcraft: 是否使用手工crafted mask
        channel_num: handcraft模式下的通道数（8/16/32），或非handcraft模式下的保留比例（0.0-1.0）
        erase_ratio: handcraft模式下的保留比例（0.25/0.5/0.75）
    ...
    if mask_stats_file is not None and not handcraft:
        ...
        if erase_ratio == 0.25:
            ...
            raise ValueError(f"不支持的erase_ratio值: {erase_ratio}. 请使用0.25, 0.5或0.75")
    ...
    if output_file:
        # 构造输出路径，使用通道数和保留比例命名
        ratio_str = f"{int(keep_ratio*100)}"
        if mask_stats_file is not None and not handcraft:
            output_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_ratio{ratio_str}_from_stats.pth')
        else:
            output_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_ratio{ratio_str}_handcraft.pth')
        ...
        # 同时保存一个JSON版本便于查看
        if mask_stats_file is not None and not handcraft:
            json_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_ratio{ratio_str}_from_stats.json')
        else:
            json_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_ratio{ratio_str}_handcraft.json')
        json_data = {
...
def main():
    ...
    parser.add_argument(
        ...
        type=float, 
        default=0.75,
        help='保留比例（0.25/0.5/0.75）'
    )
    parser.add_argument(
        '--output', 
        type=str, 
        default="/scratch/cs/vidgen/guanjr/project/latent_wf_vae_new/causalvideovae/model/channel_name/multi_keep_ratio/",
        help='输出文件路径（.pth格式）'
    )
