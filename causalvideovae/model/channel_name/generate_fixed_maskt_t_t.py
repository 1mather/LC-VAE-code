...
def generate_fixed_mask_from_stats(mask_stats_file, handcraft, channel_num=32, output_file=None):
    """
    从统计数据生成固定的通道mask
    
    Args:
        mask_stats_file: 统计数据JSON文件路径
        channel_num: handcraft模式下的通道数（8/16/32），或非handcraft模式下的保留比例（0.0-1.0）
    ...
    if mask_stats_file is not None and not handcraft:
        ...
        # 在非handcraft模式下，channel_num作为保留比例（0.0-1.0）
        keep_ratio = channel_num if channel_num <= 1.0 else channel_num / num_channels
        ...
        # handcraft模式：channel_num表示通道数（8/16/32）
        print(f"\n{'='*80}")
        print(f"生成手工crafted通道Mask")
        print(f"{'='*80}")
        print(f"通道配置: {channel_num} channels per block")
        
        if channel_num == 8:
            block_0=np.zeros(8, dtype=np.float32)
            for i in range(8):
                block_0[i] = 1.0
                
            block_1=np.zeros(8, dtype=np.float32)
            for i in range(8):
                block_1[i] = 1.0

            block_2=np.zeros(8, dtype=np.float32)
            for i in range(8):
                block_2[i] = 1.0

            block_3=np.zeros(8, dtype=np.float32)
            for i in range(0):
                block_3[i] = 1.0

                
            block_4=np.zeros(8, dtype=np.float32)
            for i in range(8):
                block_4[i] = 1.0

            block_5=np.zeros(8, dtype=np.float32)
            for i in range(0):
                block_5[i] = 1.0

            block_6=np.zeros(8, dtype=np.float32)
            for i in range(0):
                block_6[i] = 1.0

            block_7=np.zeros(8, dtype=np.float32)
            for i in range(0):
                block_7[i] = 1.0
            ...
            block_0=np.zeros(16, dtype=np.float32)
            for i in range(16):
                block_0[i] = 1.0
                
            block_1=np.zeros(16, dtype=np.float32)
            for i in range(16):
                block_1[i] = 1.0

            block_2=np.zeros(16, dtype=np.float32)
            for i in range(16):
                block_2[i] = 1.0

            block_3=np.zeros(16, dtype=np.float32)
            for i in range(0):
                block_3[i] = 1.0

                
            block_4=np.zeros(16, dtype=np.float32)
            for i in range(16):
                block_4[i] = 1.0

            block_5=np.zeros(16, dtype=np.float32)
            for i in range(0):
                block_5[i] = 1.0

            block_6=np.zeros(16, dtype=np.float32)
            for i in range(0):
                block_6[i] = 1.0

            block_7=np.zeros(16, dtype=np.float32)
            for i in range(0):
                block_7[i] = 1.0
            ...
            raise ValueError(f"Unsupported channel_num={channel_num} for handcraft mode. "
                           f"Supported values: 8, 16, 32")
        ...
        keep_ratio = num_keep / num_channels  # 计算实际保留比例
        ...
        print(f"保留比例: {keep_ratio:.1%}")
        print(f"\n各block保留情况:")
        block_names = ['LLL', 'LLH', 'LHL', 'LHH', 'HLL', 'HLH', 'HHL', 'HHH']
        blocks = [block_0, block_1, block_2, block_3, block_4, block_5, block_6, block_7]
        for i, (name, block) in enumerate(zip(block_names, blocks)):
            kept = int(block.sum())
            total = len(block)
            print(f"  Block {i} ({name}): {kept}/{total} ({kept/total*100:.1f}%)")
    ...
    if output_file:
        # 构造输出路径，使用通道数命名
        if mask_stats_file is not None and not handcraft:
            output_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_from_stats.pth')
        else:
            output_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_handcraft.pth')
        ...
        # 同时保存一个JSON版本便于查看
        if mask_stats_file is not None and not handcraft:
            json_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_from_stats.json')
        else:
            json_path = os.path.join(output_file, f'fixed_mask_{channel_num}ch_handcraft.json')
        json_data = {
...
def main():
    ...
    parser.add_argument(
        '--output', 
        type=str, 
        default="/scratch/cs/vidgen/guanjr/project/latent_wf_vae_new/causalvideovae/model/channel_name/wan_vae_32/",
        help='输出文件路径（.pth格式）'
    )
    ...
    if args.output is None:
        ...
        if args.handcraft:
            args.output = output_dir / f'fixed_mask_channel{args.channel_num}.pth'
        else:
            args.output = output_dir / f'fixed_mask_keep{args.channel_num:.2f}.pth'
