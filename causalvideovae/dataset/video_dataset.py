...
class TrainVideoDataset(data.Dataset):
    ...
    def decord_read(self, path): #dynamic_sample:
        ...
        video_data = torch.from_numpy(video_data)#([25, 262, 352, 3])
        video_data = video_data.permute(0, 3, 1, 2)
        return video_data #([25, 3, 262, 352])
...
class ParquetVideoDataset(data.Dataset):
    """
    Dataset for reading videos from parquet file with random frame sampling
    
    Parquet file should contain at least one column with video paths.
    """
    ...
    def __init__(
        ...
        """
        Args:
            parquet_path: Path to the parquet file containing video information
            sequence_length: Number of frames to sample from each video
            resolution: Target resolution for resizing
            sample_rate: Frame sampling rate (stride)
            dynamic_sample: If True, randomly vary the sample rate
            video_column: Name of the column containing video paths in the parquet file
        ...
        self.transform = transforms.Compose(
            [
                ...
                Resize(self.resolution),
                CenterCropVideo(self.resolution),
                Lambda(lambda x: 2.0 * x - 1.0),  # Normalize to [-1, 1]
            ]
        )
        ...
        # Validate video column exists
        if self.video_column not in self.df.columns:
            raise ValueError(
                f"Column '{self.video_column}' not found in parquet file. "
                f"Available columns: {list(self.df.columns)}"
            )
        
        # Filter out invalid paths
        self.df = self.df[self.df[self.video_column].notna()].reset_index(drop=True)
        print(f"Loaded {len(self.df)} videos from parquet file")
    ...
    def __getitem__(self, idx):
        """
        Get a video sample with random frame sampling
        
        Returns:
            dict: {'video': tensor of shape (C, T, H, W), 'label': metadata}
        """
        ...
        try:
            ...
            video = self.transform(video)  # T C H W -> T C H W
            video = video.transpose(0, 1)  # T C H W -> C T H W
            return dict(video=video, label=metadata)
        except Exception as e:
            print(f"Error loading video {video_path}: {e}")
            # Recursively try another random sample
            return self.__getitem__(random.randint(0, self.__len__() - 1))
    
    def decord_read(self, path):
        """
        Read video using decord with random temporal sampling
        
        Args:
            path: Path to video file
            
        Returns:
            video_data: Tensor of shape (T, C, H, W)
        """
        decord_vr = self.v_decoder(path)
        total_frames = len(decord_vr)
        ...
        # Random sample rate if dynamic sampling is enabled
        if self.dynamic_sample:
            sample_rate = random.randint(1, self.sample_rate)
        else:
            sample_rate = self.sample_rate
        ...
        # If video is too short, adjust sampling
        if total_frames < size:
            # Use available frames with smaller stride
            sample_rate = max(1, total_frames // self.sequence_length)
            size = self.sequence_length * sample_rate
        ...
        # Read frames
        video_data = decord_vr.get_batch(frame_indice).asnumpy()
        video_data = torch.from_numpy(video_data)  # (T, H, W, C)
