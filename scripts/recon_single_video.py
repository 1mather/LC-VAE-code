...
def main(args: argparse.Namespace):
    ...
    with torch.no_grad():
        ...
        encode_result = vae.encode(x_vae)
        video_recon = None
        # Prefer WFVAE_distangle-style outputs with coeffs
        coeffs_low, coeffs_high = None, None
        if hasattr(encode_result, "extra_output"):
            extra = encode_result.extra_output
            if isinstance(extra, (tuple, list)) and len(extra) >= 2:
                coeffs_low, coeffs_high = extra[0], extra[1]

        if coeffs_low is not None and coeffs_high is not None:
            video_recon = vae.decode(coeffs_low, coeffs_high).sample
        else:
            # Backward-compatible path: decode from standard latent tensor
            latents = encode_result.latent_dist.sample()
